...
export const bookingService = {
  getMyBookings: (params?: BookingListParams): Promise<PaginatedResponse<Booking>> =>
    apiClient.get('/bookings/me', { params })
      .then((r) => applyClientFilter(normalizePaginatedResponse(r.data), params)),

  getDoctorBookings: (params?: BookingListParams): Promise<PaginatedResponse<Booking>> =>
    apiClient.get('/bookings/doctor', { params })
      .then((r) => applyClientFilter(normalizePaginatedResponse(r.data), params)),

  getAllBookings: (params?: BookingListParams): Promise<PaginatedResponse<Booking>> =>
    apiClient.get('/bookings/all', { params })
      .then((r) => applyClientFilter(normalizePaginatedResponse(r.data), params)),

  create: (data: import('../types/booking.types').CreateBookingInput): Promise<Booking> =>
    apiClient.post('/bookings', data).then((r) => normalizeBooking(r.data)),
  ...
  getAvailableSlots: (doctorId: number, date: string): Promise<string[]> =>
    apiClient
      .get('/bookings/available-slots', { params: { doctor_id: doctorId, date } })
      .then((r) => r.data),
  ...
  getDailyDensity: (
    ...
    apiClient
      .get('/bookings/doctor/daily-density', { params: { start, end } })
      .then((r) => r.data),
};
