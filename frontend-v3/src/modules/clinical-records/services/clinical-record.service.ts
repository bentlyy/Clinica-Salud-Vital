...
export const clinicalRecordService = {
  async list(params: ClinicalRecordListParams = {}): Promise<PaginatedResponse<ClinicalRecord>> {
  ...
  async getById(id: number): Promise<ClinicalRecord> {
    const { data } = await apiClient.get<ClinicalRecord>(`/clinical-records/${id}`);
    return data;
  },
  ...
  async create(input: CreateClinicalRecordInput): Promise<ClinicalRecord> {
    const { data } = await apiClient.post<ClinicalRecord>('/clinical-records', input);
    return data;
  },

  async update(id: number, input: UpdateClinicalRecordInput): Promise<ClinicalRecord> {
    const { data } = await apiClient.patch<ClinicalRecord>(`/clinical-records/${id}`, input);
    return data;
  },
