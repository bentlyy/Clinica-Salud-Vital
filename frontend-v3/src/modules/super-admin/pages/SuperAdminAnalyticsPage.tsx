...
export default function SuperAdminAnalyticsPage() {
  ...
  const kpiStats: KpiStat[] = [
    ...
    },
    {
      ...
      label: 'Churn mensual',
      color: (d.churn_rate ?? 0) > 5 ? '#ef4444' : '#22c55e',
    },
    {
      icon: '🔄',
        ...
        {kpiStats.map((stat) => (
          <Grid key={stat.label} xs={12} sm={6} md={3}>
            ...
            >
              ...
              <Typography
                variant="h5"
                sx={{ fontWeight: 700, color: stat.color ?? '#1f2937' }}
              >
                {stat.value}
              </Typography>
...
function EmptyChart({ message }: { message: string }) {
  return (
    <Typography sx={{ color: '#9ca3af', textAlign: 'center', py: 5 }}>
      {message}
    </Typography>
  );
}
              ...
              {revenue.length > 0 ? (
                <ResponsiveContainer width="100%" height="100%">
                  <AreaChart data={revenue}>
                    ...
                    <XAxis dataKey="month" tick={{ fontSize: 11, fill: '#6b7280' }} tickFormatter={formatMonth} />
                    <YAxis tick={{ fontSize: 11, fill: '#6b7280' }} tickFormatter={(v: number) => `$${String(v)}`} />
...
function RevenueTab({ revenue, churn, formatMonth }: RevenueTabProps) {
  ...
  const barColors = ['#2563eb', '#7c3aed', '#d97706'];
    ...
    <>
      ...
      <Grid container spacing={2} sx={{ mb: 3 }}>
        {[
          ...
          { value: `${String(churn.churn_rate ?? 0)}%`, label: 'Churn Rate', color: (churn.churn_rate ?? 0) > 5 ? '#ef4444' : '#22c55e', textAlign: 'center' as const },
          ...
          <Grid key={stat.label} xs={12} sm={6} md={3}>
            <Paper sx={{ p: 2.5, border: '1px solid #e5e7eb', borderRadius: '12px', textAlign: stat.textAlign }}>
              <Typography variant="h5" sx={{ fontWeight: 700, color: stat.color ?? '#1f2937' }}>
                {stat.value}
              </Typography>
              <Typography variant="caption" sx={{ color: '#6b7280' }}>
                {stat.label}
    ...
    <>
      ...
      <Grid container spacing={2} sx={{ mb: 3 }}>
        {[
          ...
          { value: String(operations.total_bookings_period ?? 0), label: 'Citas (últimos 6m)', color: '#d97706' },
        ].map((stat) => (
          <Grid key={stat.label} xs={12} sm={6} md={3}>
            <Paper sx={{ p: 2.5, border: '1px solid #e5e7eb', borderRadius: '12px', textAlign: 'center' }}>
          ...
          <Box sx={{ p: 2, height: 320 }}>
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={occupancy}>
                <CartesianGrid strokeDasharray="3 3" stroke="#f3f4f6" />
                <XAxis dataKey="tenant_name" tick={{ fontSize: 11, fill: '#6b7280' }} />
                <YAxis tick={{ fontSize: 11, fill: '#6b7280' }} tickFormatter={(v: number) => `${String(v)}%`} />
