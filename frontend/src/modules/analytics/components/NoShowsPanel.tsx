  ...
  return (
    <Box>
      <Box sx={{ display: 'grid', gridTemplateColumns: { xs: '1fr 1fr 1fr' }, gap: 2, mb: 3 }}>
        <StatCard label="Citas Totales" value={total} color={theme.palette.text.primary} />
        <StatCard label="No-Shows" value={noShows} color={theme.palette.error.main} />
        <StatCard label="Tasa de No-Show" value={`${rate}%`} color={Number(rate) > 15 ? theme.palette.error.main : theme.palette.success.main} />
      </Box>
      ...
      <Paper sx={{ p: 3, border: `1px solid ${theme.palette.divider}`, mb: 3 }}>
        ...
        <ResponsiveContainer width="100%" height={300}>
          <BarChart data={data}>
            ...
            <Tooltip />
            <Legend />
            <Bar dataKey="total" fill={theme.palette.info.main} name="Total Citas" />
            <Bar dataKey="noShows" fill={theme.palette.error.main} name="No-Shows" />
          </BarChart>
        </ResponsiveContainer>
      </Paper>
