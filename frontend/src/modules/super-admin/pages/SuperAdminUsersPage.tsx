...
export default function SuperAdminUsersPage() {
  ...
  const ROLE_COLORS: Record<string, { bg: string; fg: string }> = {
    admin: { bg: theme.palette.info.light, fg: theme.palette.info.main },
    doctor: { bg: theme.palette.success.light, fg: theme.palette.success.main },
    patient: { bg: '#ede9fe', fg: '#7c3aed' },
    lab_technician: { bg: '#ffedd5', fg: '#ea580c' },
    superadmin: { bg: theme.palette.error.light, fg: theme.palette.error.main },
  };
        ...
        <Paper sx={{ borderRadius: '14px', border: `1px solid ${theme.palette.divider}`, overflow: 'hidden' }}>
          <TableContainer>
            <Table>
              ...
              <TableBody>
                {users.map((user) => {
                    const rc = ROLE_COLORS[user.role] ?? { bg: '#ede9fe', fg: theme.palette.primary.main };
                  return (
                    <TableRow key={user.id} hover>
                      ...
                      <TableCell>
                        <Chip
                          ...
                          size="small"
                          sx={{
                            backgroundColor: user.active !== false ? theme.palette.success.light : theme.palette.error.light,
                            color: user.active !== false ? theme.palette.success.main : theme.palette.error.main,
                            fontWeight: 600,
                          }}
                        />
                      </TableCell>
                      <TableCell align="right">
                        <IconButton
                          ...
                          sx={{ color: user.active !== false ? theme.palette.error.main : theme.palette.success.main }}
                        >
                          {user.active !== false
