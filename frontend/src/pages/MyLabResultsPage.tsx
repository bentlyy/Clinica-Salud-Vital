...
export default function MyLabResultsPage() {
      ...
      <PageContainer maxWidth="xl">
        <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 24 }}>
          ...
          <div>
            ...
            <p style={{ margin: '2px 0 0', fontSize: 13, color: 'var(--ds-text-secondary)' }}>Laboratorio y Exámenes</p>
          </div>
        </div>
  ...
  return (
    <PageContainer maxWidth="xl">
      <PageHeader title={t('lab_results.title')} subtitle={t('lab_results.subtitle')} />

      <div className="grid grid-3" style={{ gap: 14, marginBottom: 32 }}>
        <Card padding="md">
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
            <div>
              <p style={{ margin: 0, fontSize: 12, color: 'var(--ds-text-secondary)', textTransform: 'uppercase', letterSpacing: '0.3px' }}>Total Solicitudes</p>
        ...
        <Card padding="md">
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
            <div>
              <p style={{ margin: 0, fontSize: 12, color: 'var(--ds-text-secondary)', textTransform: 'uppercase', letterSpacing: '0.3px' }}>Completados</p>
        ...
        <Card padding="md">
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
            <div>
              <p style={{ margin: 0, fontSize: 12, color: 'var(--ds-text-secondary)', textTransform: 'uppercase', letterSpacing: '0.3px' }}>Pendientes</p>
        ...
        <div style={{ marginBottom: 32 }}>
          ...
          <div className="grid" style={{ gap: 14 }}>
            {pending.map((r) => {
              ...
              return (
                <LabCard key={r.id} request={r} icon={<Icon size={26} color={color} />} color={color} statusLabel={STATUS_LABEL[r.status] || 'Pendiente'} badgeClass={STATUS_BADGE[r.status] || 'warning'} onClick={() => navigate(`/my-lab-results/${r.id}`)} t={t} section="pending" />
              );
            })}
          </div>
        </div>
        ...
        <div>
          ...
          <div className="grid" style={{ gap: 14 }}>
            {completed.map((r) => {
              ...
              const color = getLabColor(testName);
              return (
                <LabCard key={r.id} request={r} icon={<Icon size={26} color={color} />} color={color} statusLabel={STATUS_LABEL[r.status] || 'Completado'} badgeClass={STATUS_BADGE[r.status] || 'success'} onClick={() => navigate(`/my-lab-results/${r.id}`)} t={t} section="completed" />
              );
            })}
          </div>
        </div>
      ...
      <div style={{ marginTop: 28, padding: '14px 18px', background: '#f8fafc', borderRadius: 10, display: 'flex', justifyContent: 'space-between', alignItems: 'center', fontSize: 13, color: 'var(--ds-text-secondary)' }}>
...
function LabCard({ request: r, icon, color, statusLabel, badgeClass, onClick, t, section }) {
  ...
  return (
    <Card padding="none" style={{ cursor: 'pointer', overflow: 'hidden', border: 'none', boxShadow: isPending ? '0 1px 3px rgba(0,0,0,0.06)' : '0 1px 4px rgba(0,0,0,0.04)', transition: 'all 0.15s' }}
      onClick={onClick}
      ...
      <div style={{ display: 'flex' }}>
        ...
        <div style={{ flex: 1, padding: '16px 18px', minWidth: 0 }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', gap: 8, marginBottom: 6 }}>
            ...
            <Badge variant={badgeClass} style={{ flexShrink: 0 }}>{statusLabel}</Badge>
          </div>
          <div style={{ fontSize: 13, color: 'var(--ds-text-secondary)', display: 'flex', flexWrap: 'wrap', gap: 14, marginBottom: showItems ? 10 : 0 }}>
          ...
          {showItems && (
            <div style={{ display: 'flex', gap: 4, flexWrap: 'wrap' }}>
              ...
              {items.length > 4 && (
                <span style={{ fontSize: 11, padding: '3px 10px', borderRadius: 12, background: '#f1f5f9', color: 'var(--ds-text-tertiary)' }}>
                  +{items.length - 4}
                </span>
              )}
            </div>
