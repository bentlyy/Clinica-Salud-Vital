...
Frontend React :5173  ──/api──►  Express Backend :3000  ──SQL──►  PostgreSQL 15 (RLS)
                                      │
                               SendGrid / SMTP · Twilio · Mercado Pago (CLP, stub sin token) · Sentry
```
...
| `shared/` (20) | db (pg pool), jwt, i18n, email, crypto, mercadopago, queue (tabla jobs), multi-tenant, rut, sanitize, ownership, sessions, booking-utils, booking-history, query, escape, date, base32, seed-status, sentry |
...
| SaaS | `/api/saas` | Planes, onboarding, webhook Mercado Pago |
...
**Largo plazo:** portal de autoservicio avanzado, app mobile, FHIR R4, suscripciones recurrentes Mercado Pago
