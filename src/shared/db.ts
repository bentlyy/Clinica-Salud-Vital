...
// cross-tenant superadmin panel can read all tenants regardless of RLS.
// Falls back to the main pool when DATABASE_URL_SUPERADMIN is not set.
const superAdminUrl = process.env.DATABASE_URL_SUPERADMIN;
export const superAdminPool = superAdminUrl
  ? new Pool({
      connectionString: superAdminUrl,
      ssl: sslConfig,
      client_encoding: 'UTF8',
      max: Math.max(Math.floor(poolMax / 4), 2),
      idleTimeoutMillis: 30000,
      connectionTimeoutMillis: 15000,
      statement_timeout: 30000,
      query_timeout: 30000,
...
if (superAdminUrl) {
  logger.info('⚙️ Superadmin pool configured via DATABASE_URL_SUPERADMIN (BYPASSRLS)');
}
