...
export const getAllClinicalRecords = async ({ patient_id, doctor_id, status, limit = 100, offset = 0 } = {}) => {
  let query = `
    SELECT cr.*, 
...
export const getClinicalRecordsByPatient = async (patient_id) => {
  const result = await pool.query(`
    SELECT cr.*, 
           d.name AS doctor_name, d.specialty
    FROM clinical_records cr
...
export const updateClinicalRecord = async (id, data, doctor_id) => {
  const client = await pool.connect();
...
export const deleteClinicalRecord = async (id, doctor_id) => {
  const result = await pool.query(
