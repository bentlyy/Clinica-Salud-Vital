...
export const getDbReport = asyncHandler(async (_req: Request, res: Response) => {
  const stats = await dbMonitor.getStats();
  const poolHealth = await dbMonitor.getPoolHealth();
  res.json({ ...stats, poolHealth });
});

export const getDbSlowQueries = asyncHandler(async (_req: Request, res: Response) => {
...
export const getDbTableSizes = asyncHandler(async (_req: Request, res: Response) => {
  const { rows } = await pool.query(`
    SELECT
...
export const exportDbSizes = asyncHandler(async (_req: Request, res: Response) => {
