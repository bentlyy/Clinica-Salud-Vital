...
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_bookings_doctor_date_status
  ON bookings(doctor_id, date, status)
  WHERE status != 'cancelled';

-- 2. Index for doctor daily agenda
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_bookings_doctor_date_time
...
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_refresh_tokens_token_active
  ON refresh_tokens(token, revoked, expires_at)
  WHERE revoked = false AND expires_at > NOW();

-- 11. Doctor availability lookup
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_doctor_availability_doctor_day
  ON doctor_availability(doctor_id, day_of_week);

-- 12. Doctor exceptions lookup
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_doctor_exceptions_doctor_date
