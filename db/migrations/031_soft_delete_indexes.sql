...
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_bookings_doctor_date_active
  ON bookings (doctor_id, date)
  WHERE status != 'cancelled';

-- Bookings: búsqueda por tenant + fecha (slot availability)
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_bookings_tenant_date
  ON bookings (tenant_id, date, time)
  WHERE status != 'cancelled';

-- Clinical records: búsqueda por paciente + tenant
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_clinical_records_patient_tenant
  ON clinical_records (patient_id, tenant_id, created_at DESC);

-- Clinical records: búsqueda por doctor + tenant
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_clinical_records_doctor_tenant
...
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_availability_doctor_day
  ON doctor_availability (doctor_id, day_of_week, tenant_id);

-- Doctor exceptions: date lookup
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_exceptions_doctor_date
  ON doctor_exceptions (doctor_id, date, tenant_id);

-- Invoices: búsqueda por paciente
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_invoices_patient_tenant
...
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_phi_access_log_tenant
