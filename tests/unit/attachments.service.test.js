...
const ownershipRow = { patient_id: 3, doctor_id: null };
...
describe('uploadAttachment', () => {
  it('throws for invalid entity type', async () => {
    await expect(uploadAttachment(3, 't', { ...validInput, entity_type: 'nope' }, 'patient')).rejects.toThrow(BadRequestError);
  });

  it('throws for disallowed mime type', async () => {
    await expect(uploadAttachment(3, 't', { ...validInput, mime_type: 'application/x-msdownload' }, 'patient')).rejects.toThrow(BadRequestError);
  });

  it('throws when base64 is missing', async () => {
    await expect(uploadAttachment(3, 't', { ...validInput, data_base64: '' }, 'patient')).rejects.toThrow(BadRequestError);
  });
  ...
  it('writes the file to disk and returns the record', async () => {
    ...
    const record = await uploadAttachment(3, 't', validInput, 'patient');
  ...
  it('defaults missing file name to "file"', async () => {
    mockQuery
      .mockResolvedValueOnce({ rows: [ownershipRow] })
      .mockResolvedValueOnce({ rows: [recordRow] });
    await uploadAttachment(3, 't', { ...validInput, file_name: '' }, 'patient');
    expect(mockQuery).toHaveBeenCalledWith(
      expect.any(String),
...
describe('listAttachments', () => {
  it('throws for invalid entity type', async () => {
    await expect(listAttachments('bad', 4, 't', 3, 'patient')).rejects.toThrow(BadRequestError);
  });
  ...
  it('maps rows', async () => {
    mockQuery
      .mockResolvedValueOnce({ rows: [ownershipRow] })
      .mockResolvedValueOnce({ rows: [recordRow] });
    const rows = await listAttachments('clinical_record', 4, 't', 3, 'patient');
...
describe('getAttachment', () => {
  it('throws when not found', async () => {
    ...
    await expect(getAttachment(1, 't', 3, 'patient')).rejects.toThrow(NotFoundError);
  });
  ...
  it('returns the record and file path', async () => {
    mockQuery
      .mockResolvedValueOnce({ rows: [recordRow] })
      .mockResolvedValueOnce({ rows: [ownershipRow] });
    const { record, filePath } = await getAttachment(1, 't', 3, 'patient');
    expect(record.id).toBe(1);
...
describe('deleteAttachment', () => {
  ...
  it('forbids a non-owner patient', async () => {
    mockQuery
      .mockResolvedValueOnce({ rows: [{ ...recordRow, uploaded_by: 9 }] })
      .mockResolvedValueOnce({ rows: [{ patient_id: 99, doctor_id: null }] });
    await expect(deleteAttachment(1, 't', 3, 'patient')).rejects.toThrow(ForbiddenError);
  });

  it('allows the owner to delete', async () => {
    mockQuery
      .mockResolvedValueOnce({ rows: [{ ...recordRow, uploaded_by: 3 }] })
      .mockResolvedValueOnce({ rows: [ownershipRow] })
      .mockResolvedValueOnce({ rowCount: 1 });

    await expect(deleteAttachment(1, 't', 3, 'patient')).resolves.toBeUndefined();
